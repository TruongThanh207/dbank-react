...
contract('dBank', ([deployer, user]) => {
  ...
  describe('testing borrow...', () => {
    ...
    describe('success', () => {
      ...
      it('token total supply should increase', async () => {
        expect(Number(await token.totalSupply())).to.eq(7.5*(10**15)) //10**16/2
      })

      it('balance of user should increase', async () => {
        expect(Number(await token.balanceOf(user))).to.eq(7.5*(10**15)) //10**16/2
      })
  ...
  describe('testing payOff...', () => {
    ...
    describe('success', () => {
      beforeEach(async () => {
        await dbank.borrow({value: 10**16, from: user}) //0.01 ETH
        await token.approve(dbank.address, (7.5*(10**15)).toString(), {from: user})
